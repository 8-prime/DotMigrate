...
public class MigratorTests
{
    private class TestProvider : IMigrationDatabaseProvider
    {
        ...
        public Task<int> GetVersionAsync(CancellationToken cancellationToken = default) => Task.FromResult(Version);
        public void GetLock() { }
        public Task GetLockAsync(CancellationToken cancellationToken = default) => Task.CompletedTask;
        public void ReleaseLock() { }
        public Task ReleaseLockAsync(CancellationToken cancellationToken = default) => Task.CompletedTask;
        public void ApplyMigration(IMigration migration) => _applied.Add(migration);
        public Task ApplyMigrationAsync(IMigration migration, CancellationToken cancellationToken = default) { _applied.Add(migration); return Task.CompletedTask; }
    ...
    private class TestSource : IMigrationSource
    {
        ...
        public Task<IEnumerable<IMigration>> GetMigrationsAsync(CancellationToken cancellationToken) => Task.FromResult(_migrations);
        public IMigration? Latest() => _migrations.OrderByDescending(m => m.Index).FirstOrDefault();
        public Task<IMigration?> LatestAsync(CancellationToken cancellationToken) => Task.FromResult(Latest());
    }

    private static IMigration MakeMigration(int index, string name = "m") => new FileMigration { Index = index, Name = name + index, Command = "select 1" };
    ...
    public void AllMigrationsApplied_ReturnsTrueWhenSame()
    {
        ...
        var options = new MigrationOptions<string> { Provider = provider, Source = new TestSource(migrations), Configuration = new TestConfig { ConnectionString = "x" } };
    ...
    public void Run_Validate_ThrowsWhenNotAllApplied()
    {
        ...
        var options = new MigrationOptions<string> { Provider = provider, Source = new TestSource(migrations), Configuration = new TestConfig { ConnectionString = "x", Mode = MigrationMode.Validate } };
        var migrator = new Migrator<string>(options);
    ...
    private class TestConfig : AMigrationConfiguration { public TestConfig() { ConnectionString = "x"; } }
